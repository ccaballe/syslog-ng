...
log_reader_init_watches(LogReader *self)
{
	...
	self->idle_timer.handler = log_reader_io_process_input;
...
{
	GIOCondition cond;
	// TODO @ccaballe get time from config
	gint idle_timeout = 10;
...
log_reader_handle_line(LogReader *self, const guchar *line, gint length, LogTransportAuxData *aux)
{
	...
	if (line[0] == '\n'){
		line = line + 1;
		msg_debug("CC NOW: Incoming log entry",
				evt_tag_printf("line", "%.*s", length, line));
	}
...
log_reader_fetch_log(LogReader *self)
{
	...
	while (msg_count < self->options->fetch_limit && !main_loop_worker_job_quit())
	{
		...
		msg_debug("[CC] MSG_STRING BEFORE: ", evt_tag_printf("msg", msg));
		status = log_proto_server_fetch(self->proto, &msg, &msg_len, &may_read, &aux, bookmark);
		msg_debug("[CC] MSG_STRING AFTER: ", evt_tag_printf("msg", msg));

//		if (msg[0] == '/n' && msg[1] == '/n') {
//			msg = msg + 1;
//			msg_debug("[CC] MSG_STRING AFTER AFTER: ", evt_tag_printf("msg", msg));
//		}
		...
		default:
			g_assert_not_reached();
			msg_error("[CC] DEFAULT_BREAK");
			break;
		}

		if (!msg)
		{
			msg_error("[CC] NO MESSAGES ->_BREAK");
