		...
		const guchar *buffer_start, gsize buffer_bytes, const guchar *eol, const guchar **msg, gsize *msg_len)
{
	...
	*msg = buffer_start;
	...
	{
		msg_debug("[CC logproto-text]: verdict & LPT_EXTRACTED");

		if (verdict & LPT_CONSUME_LINE)
		{
			msg_debug("CC: verdict & LPT_CONSUME_LINE");
		...
		{
			msg_debug("[CC logproto-text]: verdict & LPT_REWIND_LINE");
	...
	{
		msg_debug("[CC logproto-text]: verdict & LPT_WAITING");
		...
		*msg_len = 0;
		if (verdict & LPT_CONSUME_LINE)
		{
			msg_debug("[CC logproto-text]: verdict & LPT_CONSUME_LINE");
